...
public:
  ...
  {
    ts = static_cast<int>(tasks.size());
    ws = static_cast<int>(workers.size());

    sort(tasks.begin(), tasks.end());
    sort(workers.begin(), workers.end());

    int ans{};
    for (int l = 0, r = min(ts, ws), m; l <= r;)
    {
      // m中点，一定要完成的任务数量
      m = l + (r - l) / 2;
      if (f(tasks, 0, m - 1, workers, ws - m, ws - 1, pills, strength))
      {
        ans = m;
        l = m + 1;
      }
  ...
  bool f(vector<int> &tasks, int tl, int tr,
         vector<int> &workers, int wl, int wr,
