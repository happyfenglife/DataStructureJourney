...
int value1(vector<vector<int>> &dp, int i, int j)
{
...
int compute1()
{
  ...
  for (int i = 1; i <= n; i++)
  {
    ...
    for (int mod = 0; mod <= min(t, w[i] - 1); mod++)
    {
      ...
      {
        while (l < r && value1(dp, i, que[r - 1]) <= value1(dp, i, j))
        {
          --r;
        }

        que[r++] = j;
        if (que[l] == j - w[i] * (c[i] + 1))
        {
          ++l;
        }

        dp[i][j] = value1(dp, i, que[l]) + j / w[i] * v[i];
      }
    }
  }
...
int compute2()
{
  for (int i = 1; i <= n; i++)
  {
    for (int mod = 0; mod <= min(t, w[i] - 1); mod++)
    {
      ...
      {
        while (l < r && value2(i, que[r - 1]) <= value2(i, j))
        {
          r--;
        }
      ...
      for (int j = t - mod, enter = j - w[i] * c[i]; j >= 0; j -= w[i], enter -= w[i])
      {
        ...
        if (enter >= 0)
        {
          while (l < r && value2(i, que[r - 1]) <= value2(i, enter))
          {
            --r;
          }
