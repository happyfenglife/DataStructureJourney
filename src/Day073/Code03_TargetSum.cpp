...
public:
  ...
  int dfs1(vector<int> &nums, int target, int i, int sum)
  {
    if (i == nums.size())
    {
    ...
    return dfs1(nums, target, i + 1, sum - nums[i]) + dfs1(nums, target, i + 1, sum + nums[i]);
  }
  ...
  int dfs2(vector<int> &nums, int target, int i, int sum, unordered_map<int, unordered_map<int, int>> &dp)
  {
    if (i == nums.size())
    {
      return sum == target ? 1 : 0;
    ...
    int ans = dfs2(nums, target, i + 1, sum + nums[i], dp) + dfs2(nums, target, i + 1, sum - nums[i], dp);
    return dp[i][sum] = ans;
  }
